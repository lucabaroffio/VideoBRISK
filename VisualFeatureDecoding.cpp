...
int VisualFeatureDecoding::decodeBinaryDescriptors(string descName, vector<uchar> bitstream, Mat &features, int nFeats)
{
	...
	if( strcmp(descName.c_str(),"BRISK")==0 ){
		...
		Mat features_flipped;
		if( decodeBRISK(bitstream,features_flipped,nFeats) == 0 ){

			features = features_flipped.clone();
			for(int r=0; r<features_flipped.rows; r++){
				for(int i=0; i<64; i++){
					int curData = (int)features.row(r).at<uchar>(i);
					int revData = (curData & 0x80) >> 7 |
								  ...
								  (curData & 0x02) << 5 |
								  (curData & 0x01) << 7;
					features.row(r).at<uchar>(i) = (uchar)revData;
				}
			}
...
int VisualFeatureDecoding::dummy_decodeBinaryDescriptors(string descName, vector<uchar> bitstream, Mat& features)
{
	...
	int descSize = 64;
	if ( descName.compare("BRISK")==0 || descName.compare("FREAK")==0 ){
		descSize = 64;
	}
