/*
 * VisualFeatureEncodingInter.cpp
 */
...
int VisualFeatureEncoding::encodeBinaryDescriptors(string descName, Mat features, vector<uchar>& bitstream)
{
	...
	if( strcmp(descName.c_str(),"BRISK")==0 ){
		...
		Mat features_flipped = features.clone();
		for(int r=0; r<features.rows; r++){
			for(int i=0; i<features.cols; i++){
				int curData = (int)features_flipped.row(r).at<uchar>(i);
				int revData = (curData & 0x80) >> 7 |
							  (curData & 0x40) >> 5 |
							  (curData & 0x20) >> 3 |
							  (curData & 0x10) >> 1 |
							  (curData & 0x08) << 1 |
							  ...
							  (curData & 0x02) << 5 |
							  (curData & 0x01) << 7;
				features_flipped.row(r).at<uchar>(i) = (uchar)revData;
			}
		}

		//cout << features.row(0) << endl;
		//cout << features_flipped.row(0) << endl;
...
//					if(cur_bit==0)
//						cout<<"Primo bit: " << cur_bit << " con prob " << pModel->getP0(0)<< endl;
//					else
//						cout<<"Primo bit: " << cur_bit << " con prob " << 1-pModel->getP0(0)<< endl;
				}
				else{
					...
					if( prev_bit == 0){
						...
						freq[1] = AC_PRECISION - freq[0];
//						if(cur_bit==0)
//							cout<<"bit successivo: " << cur_bit << " con prob " << pModel->getP0c0(index,index-1)<< endl;
//						else
//							cout<<"bit successivo: " << cur_bit << " con prob " << 1-pModel->getP0c0(index,index-1)<< endl;
...
//						if(cur_bit==0)
//							cout<<"bit successivo: " << cur_bit << " con prob " << pModel->getP0c1(index,index-1)<< endl;
//						else
//							cout<<"bit successivo: " << cur_bit << " con prob " << 1-pModel->getP0c1(index,index-1)<< endl;
